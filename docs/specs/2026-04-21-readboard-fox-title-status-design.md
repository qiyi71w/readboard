...
追加下一手提示时，提示位于基础标题和窗口上下文之间，例如：

- `棋盘同步工具 [白] [房间][43581号][第89手]`
- `棋盘同步工具 [?] [野狐][同步中][未抓到标题信息]`

下一手提示只来自最近一次识别成功的 `BoardSnapshot.BoardState`：

- 唯一 `BlackLastMove` 表示下一手提示 `[白]`
- 唯一 `WhiteLastMove` 表示下一手提示 `[黑]`
- 无末手或多末手异常显示 `[?]`

该提示只用于人工观察和手动“交换顺序”辅助，不写入 outbound protocol，不参与自动落子、GMA 轮次信任、`foxMoveNumber` 或 history matching 决策。让子初始局、无末手局面不能用棋子数量在标题里强行猜测轮次，应显示 `[?]`。
