...
本文中的 `1100 x 680` 和 `700 x 433` 均指 Windows DPI 处理后的有效逻辑客户区尺寸（DIP / CSS 像素），不是显示面板的原始物理像素。`700 x 433` 是硬性尺寸下限。密度模式下侧栏收成图标、日志少露几行，普通按钮保持约 `32 x 32px`，窗口控制按钮约 `40 x 40px`，正文不小于 `13px`。

当前屏幕工作区本身小于 `700 x 433` 时进入紧急适配：保持密度排版的最小客户区，允许主内容区滚动兜底；工作区宽度也不足时可以出现横向滚动。该例外只用于低于最低支持工作区的环境，不能用固定坐标裁掉操作栏。

客户区大于 `1100 x 680` 时字体和图标保持基准尺寸；控制中心按钮与数字输入框可随有效高度在 `37px` 到 `48px` 间连续舒展，分组间距可在 `6px` 到 `12px` 间连续舒展，配置分组底部内边距可在 `3px` 到 `8px` 间连续舒展，避免放大后的控件边框紧贴分组外框。达到上限后只继续扩展内容宽度、列宽、留白和日志区域。WebView2 负责跟随 Windows DPI，页面不能再按 DPI 额外叠加一次放大。

小于基准尺寸时不使用整页缩放因子。窗口宽而矮或窄而高时，由密度排版吸收剩余空间。不能分别拉伸控件长宽比来填满窗口。

用户拖动窗口时按帧合并重新计算密度布局，不添加带延迟的缩放过渡动画。停止拖动后文字和图标保持清晰。
...
- 客户区小于基准尺寸时切换密度排版，不使用整页等比缩放。
- 从基准尺寸拖到最小尺寸时密度布局连续更新且无过渡动画；停止拖动后文字和图标清晰。
- 标题栏在基准尺寸下使用 `48px` 高度、`28px` 应用图标、`18px` 主标题、`13px` 版本号和三个 `48 x 48px` 窗口按钮；密度模式下标题栏约 `40px`，窗口按钮约 `40 x 40px`。
- 客户区不小于 `700 x 433` 时，侧栏的 4 个导航项和 7 个快捷操作全部同时可见；密度模式下以图标显示，不折叠、不隐藏且不出现侧栏滚动条。
