...
界面缩放只在客户区小于 `1100 x 680` 时向下生效，字体、图标、按钮和间距使用同一个缩放因子。客户区大于基准尺寸时字体和图标保持基准尺寸；控制中心按钮与数字输入框可随有效高度在 `37px` 到 `48px` 间连续舒展，分组间距可在 `6px` 到 `12px` 间连续舒展，配置分组底部内边距可在 `3px` 到 `8px` 间连续舒展，避免放大后的控件边框紧贴分组外框。达到上限后只继续扩展内容宽度、列宽、留白和日志区域。WebView2 负责跟随 Windows DPI，页面不能再按 DPI 额外叠加一次放大。
