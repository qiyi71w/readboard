...
客户区不小于 `700 x 433` 时，主窗口外层、左侧导航和控制中心不显示页面滚动条。参数设置、规则说明和关于页只在宽 `≤ 918` 或高 `< 680` 时允许页面滚动。日志输出、身份候选列表、更新说明等内容数量不固定的局部区域可以独立滚动，并使用细窄、低存在感的滚动条样式。主内容区滚动只用于屏幕工作区小于 `700 x 433` 的紧急适配。
...
- 客户区不小于 `700 x 433` 时，主窗口、左侧导航和控制中心无页面滚动条；参数设置、规则说明和关于页只在宽 `≤ 918` 或高 `< 680` 时滚动。日志、身份候选和更新说明等动态区域可在固定边界内局部滚动。
